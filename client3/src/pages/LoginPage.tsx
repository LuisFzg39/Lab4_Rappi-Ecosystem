  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (data.user.role !== 'delivery') {
        showToast('This app is for delivery drivers only', 'error');
        return;
      }
      setAuth(data);
      navigate('/available');
  ...
  return (
    <div class="min-h-screen flex items-center justify-center bg-gray-50">
      <div class="bg-white p-8 rounded-2xl shadow w-full max-w-sm">
        <h1 class="text-2xl font-semibold text-gray-800 mb-2 text-center">Delivery App</h1>
        <p class="text-sm text-gray-400 text-center mb-6">Sign in to deliver orders</p>
        ...
        <p class="text-sm text-gray-500 text-center mt-4">
          Want to deliver?{' '}
          <Link to="/register" class="text-amber-600 hover:underline">Register</Link>
        </p>
      </div>
    </div>
  );
