...
export function AvailableOrdersPage() {
  ...
  useEffect(() => { fetchOrders(); }, [axios]);

  useEffect(() => {
    console.log('[Delivery] Subscribing to orders:global');
    const channel = supabase.channel('orders:global');

    channel
      .on('broadcast', { event: 'order-created' }, (message) => {
        console.log('[Delivery] Received order-created:', message.payload);
      ...
      .on('broadcast', { event: 'order-taken' }, (message) => {
        console.log('[Delivery] Received order-taken:', message.payload);
        const payload = message.payload as { orderId: string };
        setOrders((prev) => prev.filter((o) => o.id !== payload.orderId));
      })
      .subscribe((status) => {
        console.log('[Delivery] orders:global subscription status:', status);
        setLiveStatus(status === 'SUBSCRIBED' ? 'live' : status === 'CLOSED' ? 'offline' : 'connecting');
      });

    return () => {
      console.log('[Delivery] Unsubscribing from orders:global');
    ...
    try {
      ...
      globalChannel.subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.log('[Delivery] Broadcast order-taken for', orderId);
            ...
            {orders.map((order) => (
              <div key={order.id} class="bg-white rounded-2xl shadow p-6">
                ...
                <div class="border-t border-gray-100 mt-3 pt-3 flex justify-between items-center">
                  <Link to={`/orders/${order.id}`} class="text-sm text-amber-600 hover:underline">
                    View Details
                  </Link>
