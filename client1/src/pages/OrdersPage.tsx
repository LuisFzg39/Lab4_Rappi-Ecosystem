  ...
  useEffect(() => {
    ...
    channel
      .on('broadcast', { event: 'order-taken' }, (message) => {
        console.log('[Consumer Orders] Received order-taken (global):', message.payload);
      ...
      .on('broadcast', { event: 'order-accepted' }, (message) => {
        console.log('[Consumer Orders] Received order-accepted (global):', message.payload);
        const p = message.payload as Order;
        setOrders((prev) => prev.map((ord) => (ord.id === p.id ? p : ord)));
      })
      .on('broadcast', { event: 'order-delivered' }, (message) => {
        console.log('[Consumer Orders] Received order-delivered (global):', message.payload);
        const p = message.payload as Order;
        setOrders((prev) => prev.map((ord) => (ord.id === p.id ? p : ord)));
      })
      .subscribe((status) => {
        console.log('[Consumer Orders] orders:global subscription:', status);
