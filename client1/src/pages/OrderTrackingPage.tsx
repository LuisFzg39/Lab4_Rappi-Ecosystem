...
export function OrderTrackingPage() {
  ...
  useEffect(() => {
    ...
    console.log('[Consumer] Subscribing to channel: order:' + id);
    ...
    trackingChannel
      .on('broadcast', { event: 'position-update' }, (message) => {
        ...
        console.log('[Consumer] Received position-update:', p);
        if (p && typeof p.lat === 'number' && typeof p.lng === 'number') {
          setDeliveryPos({ lat: p.lat, lng: p.lng });
        }
      })
      .subscribe((status) => {
        console.log('[Consumer] order:' + id + ' subscription status:', status);
    ...
    globalChannel
      .on('broadcast', { event: 'order-accepted' }, (message) => {
        ...
        if (!p || p.id !== id) return;
        console.log('[Consumer Tracking] order-accepted:', p);
        setOrder(p);
      ...
      .on('broadcast', { event: 'order-delivered' }, (message) => {
        const p = message.payload as Order;
        if (!p || p.id !== id) return;
        console.log('[Consumer Tracking] order-delivered:', p);
        setOrder(p);
        showToast('Your order has been delivered!', 'success');
      })
      .subscribe((status) => {
        console.log('[Consumer Tracking] orders:global subscription status:', status);
      });

    return () => {
      console.log('[Consumer] Unsubscribing from channels for order:' + id);
    ...
    <div class="min-h-screen bg-gray-50">
      <header class="bg-white shadow-sm px-6 py-4 flex justify-between items-center">
        <div class="flex items-center gap-3">
          <button onClick={() => navigate('/orders')} class="text-gray-400 hover:text-gray-600 text-sm">
            &larr; My Orders
          </button>
